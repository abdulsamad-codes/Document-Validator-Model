...
**`app/operator_workflow`** is the real first-level triage stage, sitting between Completeness and the rest of the pipeline: an operator queue (`GET /validation/applications`, business-completeness state per application — status, required/received/missing document counts) plus three OPERATOR-gated actions (`request-documents`, `operator-reject`, `operator-submit`) that move an application through `SUBMITTED`/`NEEDS_DOCUMENTS` toward processing or rejection. Every action writes an immutable validation-history entry plus the shared audit log. This is a different, unrelated module from `app/validation` (see the Frontend section below and `CONTEXT.md` for that module's fate) despite the overlapping URL prefix (`/validation/...`) — don't confuse the two from routes alone.
